...
import { SubCategoriesResponse } from "../bookcommerce.infrastructure/DTO/Responses/SubCategoriesResponse";
...
export class SubCategoryController
{
  ...
  //GET /sub-categories
  public async getSubCategoriesByVendorId(req: Request, res: Response): Promise<Response>
  {
    const result = await this.subCategoryService?.getSubCategoriesByVendorId(String(req.id))
    return res.status(200).json(new SubCategoriesResponse({
      status: true,
